...
; keep bars at same y level unless you want otherwise
(define bar-y 0.0)
...
(define gold-geometry
  (list
   ; Semi-infinite-style SiO2 substrate
   ; Extends from the lower PML region upward to just below the metal
   (make block
         (size (vector3 period period 3.0))
         (center (vector3 0 0 -1.75))
         (material SiO2))

   ; Horizontal rectangle (long in x)
   (make block
         (size (vector3 bar-length bar-width metal-z))
         (center (vector3 blk1-x blk1-y gold-z-center))
         (material gold-3term))

   ; Vertical rectangle (long in y)
   (make block
