...
def calculate_raw_rt(inc, rod):
    """
    Calculates normalized flux ratios only where the incident normalization
    flux is strong enough.
    ...
    Sign convention from your sign-check:
        R = -rod_refl / inc_refl
        T_relative = rod_trans / inc_trans

    Important:
        inc_refl and inc_trans are different normalization references because
        they come from different monitor planes in the substrate-only run.
        That means 1 - R - T_relative is not a physically valid absorption.
    """
...
def plot_rta_case(case_name, result, outdir):
    ...
    R = result["R_minus"]
    T_relative = result["T_relative"]

    plt.figure(figsize=(10, 6))
    plt.plot(wl, R, label="Backscatter fraction")
    plt.plot(wl, T_relative, label="Transmission ratio vs bare substrate")

    plt.axhline(0, linewidth=1)
    plt.xlabel("Wavelength (µm)")
    plt.ylabel("Fraction")
    plt.title(f"{case_name}: normalized response")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
for case_name, files in CASES.items():
    ...
    print(f"  T_relative:       {result['T_relative'].min():.6e} to {result['T_relative'].max():.6e}")
    print("  Note: R and T_relative use different baseline monitors, so they should not")
    print("        be combined into a derived absorption curve.")
    print()
...
plot_comparison(
    ...
    title="Nanorod validation: Backscatter fraction",
...
plot_comparison(
    all_results,
    quantity="T_relative",
    ylabel="Transmission ratio vs bare substrate",
    title="Nanorod validation: Transmission ratio vs bare substrate",
